...
def render_interactive_3d_scene(light: dict[str, Any]) -> None:
    ...
    <div style="font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      ...
      <canvas id="scene3d" width="920" height="430" style="width:100%;height:430px;border:1px solid #d4d0c8;border-radius:6px;background:#f7f5ef;display:block;"></canvas>
    </div>
    <script>
    ...
    """.replace("__SCENE__", scene_json)
    components.html(html, height=485)
...
def render_setup(data: dict[str, Any], maps: dict[str, Image.Image]) -> None:
    analysis = data["image_analysis"]
    setup = data["physical_setup"]
    ...
    with map_cols[0]:
        st.caption("明るさマップ: 明るいほど黄白")
        st.image(maps["brightness"], use_container_width=True)
    with map_cols[1]:
        st.caption("疑似深度マップ: 近いほど赤、遠いほど青")
    ...
    st.divider()
    st.subheader("ライト配置")
    render_interactive_3d_scene(data["light_plan"])
    st.caption("三方向の補助図")
    render_light_diagram(data["light_plan"])

    st.divider()
    st.subheader("制作ガイド")

    tabs = st.tabs(["空間", "主光源", "光源一覧", "物体", "カメラ", "風・動き", "外側の手順"])
    with tabs[0]:
        st.json(setup["space"])
    with tabs[1]:
        st.json(setup["light"])
    ...
    with tabs[6]:
        for step in data["outside_frame_plan"]:
...
def main() -> None:
    ...
    if input_mode == "サイト内で生成":
        with st.container(border=True):
            ...
            if generate_cols[0].button("ローカルデモ画像を生成", use_container_width=True):
        ...
        uploaded_file = st.file_uploader(
            ...
            help="画像を入れると、明るさマップ・疑似深度マップ・ライト配置・制作手順を推定します。",
        )
